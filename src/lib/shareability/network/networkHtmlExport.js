...
/* Text index of the graph. Crowded labels are hidden in the SVG and node
   selection is pointer-only, so without this the owner names in a shared file
   are neither findable nor reachable. */
function renderOwnerIndex(nodes) {
  const items = nodes
    .map((node) => {
      const name = escapeHtml(node.label);
      const link = node.profileHref
        ? `<a href="${escAttr(node.profileHref)}">${name}</a>`
        : name;
      const shared = node.isHub
        ? 'subject of this network'
        : `${node.sharedCount} shared ${node.sharedCount === 1 ? 'facility' : 'facilities'}`;
      return `<li>${link} <span>${shared}</span></li>`;
    })
    .join('');

  return (
    `<details class="owner-index">` +
    `<summary>All owners (${nodes.length})</summary>` +
    `<ul>${items}</ul></details>`
  );
}
